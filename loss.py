...
class Perplexity(NLLLoss):
    ...
    def eval_batch(self, outputs, target):
        self.acc_loss += self.criterion(outputs, target) # criterion = torch.nn.NLLLoss()
...
class EntropyLoss(nn.Module):
    ...
    def forward(self, prob_distribution):
        intermediate = prob_distribution * torch.log(prob_distribution)
        #if torch.distributed.get_rank() == 0:
        #    print(intermediate)
        entropy = torch.sum(intermediate, dim=-1)
        return torch.mean(entropy, dim=-1) * self.coefficient
